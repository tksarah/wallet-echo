...
export async function generateImage(prompt:string,style?:GenerationStyle):Promise<Buffer>{
  const c=config();if(!c.openaiKey)throw new AppError('NOT_CONFIGURED','画像生成APIが未設定です。',503);
  const raw=await fetchJson('https://api.openai.com/v1/images/generations',{method:'POST',headers:{Authorization:`Bearer ${c.openaiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model:c.imageModel,prompt,n:1,size:'1024x1024',quality:'medium',output_format:'png',...(style==='legendary-card'?{background:'transparent'}:{})})},180000);
