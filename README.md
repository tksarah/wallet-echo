...
利用者画面の正本は本リポジトリの `src/` です。Sitesの既存チェックアウトを `sites-wallet-echo/` に開いた後、`node scripts/sync-sites-ui.mjs` でページ・専用CSS・作風選択・完成見本・日英文言の6ファイルを一括反映します。`node scripts/sync-sites-ui.mjs --check` で反映漏れを確認できます。HeroのCSSだけを更新すると他の画面改善が反映されません。

同期対象にAPI・保存処理・環境設定・データは含みません。別の機能変更を同時に行う場合はページ内の処理差分を先に確認してください。Sites側で型チェック・既存テスト・本番ビルドを実施し、日英のPC・スマホ表示をステージングと照合してから正式なSites公開手順で反映します。
